...
class UpdateExerciseScreen extends StatefulWidget {
  String exerciseId;
  double time;
  UpdateExerciseScreen({Key? key, required this.exerciseId, required this.time})
      : super(key: key);

  @override
  _UpdateExerciseScreenState createState() =>
      _UpdateExerciseScreenState(exerciseId: exerciseId, time: time);
}
...
class _UpdateExerciseScreenState extends State<UpdateExerciseScreen> {
  ...
  _UpdateExerciseScreenState(
      {Key? key, required this.exerciseId, required this.time});
  ...
  Widget build(BuildContext context) {
    ...
    final UpdateExerciseButton = Material(
      elevation: 5,
  ...
  void addExerciseToDiary(String newTime) async {
    if (_formKey.currentState!.validate()) {
      Diary diary;
      String id = exercise.id!;
      double timeInSeconds = double.parse(newTime);
      double caloriesBurned =
          exercise.caloriesPerMinute! * (timeInSeconds / 60);
      ...
      await FirebaseFirestore.instance
          ...
          .then((value) => {
                ...
                diary.exercises!.remove(exerciseId),
                diary.exercises!.addAll(element),
                diary.exercise = diary.exercise! +
                    caloriesBurned -
                    exercise.caloriesPerMinute! * (time / 60),
                FirebaseFirestore.instance
                    .collection('diary')
                    .doc(loggedInUser.uid)
                    .set(diary.toMap())
